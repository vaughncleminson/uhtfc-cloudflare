...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_to_order_idx\` ON \`email_audit_logs_to\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_to_parent_id_idx\` ON \`email_audit_logs_to\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_cc_order_idx\` ON \`email_audit_logs_cc\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_cc_parent_id_idx\` ON \`email_audit_logs_cc\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_bcc_order_idx\` ON \`email_audit_logs_bcc\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_bcc_parent_id_idx\` ON \`email_audit_logs_bcc\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_updated_at_idx\` ON \`email_audit_logs\` (\`updated_at\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`email_audit_logs_created_at_idx\` ON \`email_audit_logs\` (\`created_at\`);`,
  )
  await db.run(
    sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`email_audit_logs_id\` integer REFERENCES email_audit_logs(id);`,
  )
  await db.run(
    sql`CREATE INDEX \`payload_locked_documents_rels_email_audit_logs_id_idx\` ON \`payload_locked_documents_rels\` (\`email_audit_logs_id\`);`,
  )
}
